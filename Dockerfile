...
RUN \
  ...
  dnf install --nodocs -y \
    ...
    wget \
    zip && \
  export RPM_BASE='http://mirror.centos.org/centos/8/BaseOS/x86_64/os/Packages/' && \
  wget ${RPM_BASE} -O pkglist && \
  wget -O tmux.rpm \
...
# TODO: confirm this is needed for cronie
# RUN touch /etc/crontab /etc/cron.*/*

ENV SERVER_DIR="/data" \
    BACKUP_DIR="/data/backups" \
    JAR_NAME="vanilla" \
    OPT_PARAMS="-XX:+UseG1GC -Dsun.rmi.dgc.server.gcInterval=2147483646 -XX:+UnlockExperimentalVMOptions \
